...
NSString * Gover_Zachary_AVF_WK3$ModuleRequireFormat = @"(function(exports){"
...
@implementation Gover_Zachary_AVF_WK3Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_gover_zachary_avf_wk3);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Gover_Zachary_AVF_WK3 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Gover_Zachary_AVF_WK3" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_gover_zachary_avf_wk3 = [[Gover_Zachary_AVF_WK3Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_gover_zachary_avf_wk3];
	
	NSString *_gover_zachary_avf_wk3NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _gover_zachary_avf_wk3NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_gover_zachary_avf_wk3 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Gover_Zachary_AVF_WK3$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.gover_zachary_avf_wk3.kroll" 
