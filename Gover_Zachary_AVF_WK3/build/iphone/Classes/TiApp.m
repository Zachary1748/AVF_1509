...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kGover_Zachary_AVF_WK3UserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kGover_Zachary_AVF_WK3UserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Gover_Zachary_AVF_WK3 app using the node.js CLI
