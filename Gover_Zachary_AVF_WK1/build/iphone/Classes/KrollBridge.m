...
NSString * Gover_Zachary_AVF_WK1$ModuleRequireFormat = @"(function(exports){"
...
@implementation Gover_Zachary_AVF_WK1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_gover_zachary_avf_wk1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Gover_Zachary_AVF_WK1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Gover_Zachary_AVF_WK1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_gover_zachary_avf_wk1 = [[Gover_Zachary_AVF_WK1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_gover_zachary_avf_wk1];
	
	NSString *_gover_zachary_avf_wk1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _gover_zachary_avf_wk1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_gover_zachary_avf_wk1 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Gover_Zachary_AVF_WK1$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.gover_zachary_avf_wk1.kroll" 
